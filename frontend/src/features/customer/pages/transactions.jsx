          ...
          {trans.detail_transaksi?.length > 0 && (
            <div className="bg-muted/40 rounded-xl p-3 space-y-1.5 border">
              ...
              {trans.detail_transaksi.map((d, idx) => {
                ...
                const hasReviewed = reviewInfo?.has_reviewed === true;
                const ulasanData = reviewInfo?.ulasan;
                
                return (
                  <div key={d.id_detail || idx} className="bg-card rounded-lg p-3 border space-y-2">
                    {/* Header Barang */}
                          ...
                          {hasReviewed ? (
                            <div className="flex gap-2">
                              ...
                              <Button 
                                ...
                                disabled={ulasanData?.edited_count >= 2}
                              >
                                <Edit className="w-3 h-3 mr-1" /> 
                                Edit {ulasanData?.edited_count || 0}/2
                              </Button>
                            </div>
                          ) : (
                            <Button 
                    ...
                    {/* TAMPILKAN ULASAN YANG SUDAH DIBUAT */}
                    {hasReviewed && ulasanData && (
                      <div className="bg-amber-50/30 dark:bg-amber-950/5 rounded-lg p-3 mt-2 border border-amber-100 dark:border-amber-900/20">
                        <div className="flex items-center gap-1 mb-1.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star
                              key={s}
                              className={`w-3.5 h-3.5 ${s <= (ulasanData?.rating || 0) ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}`}
                            />
                          ))}
                          <span className="text-[10px] text-muted-foreground ml-2">
                            {ulasanData?.edited_count > 0 && `(diedit ${ulasanData.edited_count}x)`}
                          </span>
                        </div>
                        {ulasanData?.komentar && (
