...
export default function Navbar({
  ...
  }, [chatContainerRef, rentalMode]);

  // Polling for active chat messages
  useEffect(() => {
    if (!rentalMode || !isChatOpen) return;
    ...
    return () => clearInterval(interval);
  }, [isChatOpen, activeChatId, rentalMode]);
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-background/95 backdrop-blur-md shadow-sm' : 'bg-background'
      }`}>
      <div className="container max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {rentalMode && (
              <>
                {/* Notification Bell - light variant for light navbar */}
                <NotificationBell variant="light" additionalNotifications={additionalNotifications} />
                ...
                <div className="relative" ref={chatContainerRef}>
                  <button
                    ...
                    className="relative p-2 text-gray-600 hover:text-emerald-600 transition duration-200 bg-transparent border-none cursor-pointer"
                  ...
                  {rentalMode && (
                    <>
                      ...
                      <button
                        ...
                        className="relative p-2 text-gray-600 hover:text-emerald-600 transition bg-transparent border-none cursor-pointer"
                      >
                        <MessageCircle className="h-5 w-5" />
                        {unreadChats > 0 && (
                          <span className="absolute top-0 right-0 flex h-4 min-w-4 px-1 items-center justify-center bg-red-500 text-white text-[9px] font-bold rounded-full animate-bounce pointer-events-none">
